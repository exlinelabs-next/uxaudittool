  ...
  return (
    <main className="flex flex-col min-h-screen" style={{ background: 'var(--wb-bg)' }}>
      ...
      <section className="flex flex-col items-center text-center px-4 pt-20 pb-12 gap-6">
        {/* Logo / brand */}
        <a href="https://webbeet.studio" className="inline-flex items-center gap-2 mb-2 opacity-80 hover:opacity-100 transition-opacity">
          <span className="text-xl font-bold tracking-tight" style={{ color: 'var(--wb-text)' }}>
        ...
        <div className="flex flex-col gap-3 max-w-xl">
          ...
          <p className="text-base sm:text-lg" style={{ color: 'var(--wb-muted)' }}>
            Get a free instant audit across SEO, performance, trust, mobile and accessibility.
            No signup required.
          </p>
        </div>
        ...
        {!hasResult && (
          <div className="flex flex-wrap justify-center gap-x-8 gap-y-2 mt-2">
            {[
              { value: '51', label: 'checks run' },
              { value: '6',  label: 'categories' },
              { value: '~20s', label: 'average time' },
            ].map(({ value, label }) => (
